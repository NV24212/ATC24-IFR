    ...
    <div class="section">
      ...
      <div class="config-group">
        ...
        <p style="color: var(--text-muted); font-size: 12px; margin-bottom: 10px;">
          If the flight plan route shows "N/A" or is unclear, use "Radar Vectors". 
          To keep the original filed route unchanged, select "As Filed".
        </p>
        <select id="routingType" onchange="handleRoutingTypeChange()">
          <option value="AS_FILED">As Filed (Use original filed route)</option>
...
<script>
  ...
  function generateClearance() {
    ...
    const rawFlightLevel = selectedFlightPlan.flightlevel || '350';
    // Ensure flight level is properly formatted with FL prefix
    const flightLevel = rawFlightLevel.startsWith('FL') ? rawFlightLevel : \`FL\${rawFlightLevel.toString().padStart(3, '0')}\`;
    ...
    switch (routingType) {
      ...
      case 'SID':
        ...
        if (sidName) {
          route = sidName.includes('departure') ? sidName : \`\${sidName} departure\`;
        } else if (!planRoute || planRoute === 'N/A' || planRoute === 'GPS Direct' || planRoute.toLowerCase() === 'direct') {
    ...
    // Generate clearance in the requested format
    const clearance = \`\${callsign} cleared to \${destination} via \${route} as filed. Climb initial \${ifl}ft. Expect \${flightLevel}. Departure frequency is \${departureFreq}. Squawk code is \${squawk}.\`;
...
>>>>>>> 199076bf638d578d6962963945c52a88ddb67d4c
